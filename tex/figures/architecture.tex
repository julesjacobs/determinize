...
\node[stage, compilation, text width=.17\textwidth] (front) at (.25,-1.05) {Parse and\\elaborate\leanref{frontend}};
\node[verified, compilation, text width=.17\textwidth] (infer) at (.48,-1.05) {Infer types\\and E/G\leanref{inference}};
\node[verified, compilation, text width=.155\textwidth] (det) at (.72,-1.05) {Determinize\leanref{determinization}};
\node[data, text width=.15\textwidth] (source) at (.91,.40) {Annotated source\\$e_{\mathrm{src}}$};
...
\draw[draw=black!65, line width=.6pt] (infer.east) -- (.605,-1.05);
...
then runs verified type and E/G inference, with a branch to the annotated source
and another through determinization.
...
measure, and the soundness theorems relate the output measures of the annotated
