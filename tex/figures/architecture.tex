...
\node[data, text width=.16\textwidth] (measure) at (.635,-3.50) {Output measure\\$\law e$};
\node[verified, text width=.16\textwidth] (soundness) at (.91,-3.50)
  {Soundness theorems\leanref{expectation-preservation}\\$\law{e_{\mathrm{src}}}$ -- $\law{e_{\mathrm{det}}}$};
\draw[flow] (semantics) -- (measure);
\draw[flow] (measure) -- (soundness);
...
measure, and the soundness theorems relate the output measures of the checked
source and the transformed program. The graph feeds either a
