...
\begin{mathpar}
  ...
  \and
  \inferrule[\textsc{Mul}]
    {\Gamma \vdash e_1 : \Float{m_1} \\ \Gamma \vdash e_2 : \Float{m_2} \\ m_1 \preceq \G \\ m_2 \preceq m}
    {\Gamma \vdash e_1 \times e_2 : \Float{m}}
  \and
  \inferrule[\textsc{Div}]
  {\Gamma \vdash e : \Float{m_1} \\ \Gamma \vdash c : \Float{m_2} \\ m_1 \preceq m \\ m_2 \preceq \G}
...
Multiplication requires a general-mode left factor and takes the mode of its right factor; addition is parametric in the mode; comparison is only allowed at general mode. Uniform sampling preserves the incoming mode; Gaussian sampling returns the incoming mode but requires a general-typed variance.
