...
\begin{definition}[Realization]
  We define \(\mathsf{realize}_\rho(\hat e)\) recursively on symbolic residual
  expressions:
  \[
  \begin{aligned}
...
hole with the expression \(e\). More formally:

\begin{definition}[Context realization]
  We define \(\mathsf{realize}_\rho(C)[e]\) recursively on symbolic evaluation contexts.
  Representative clauses are:
  \[
  \begin{aligned}
    \mathsf{realize}_\rho([\cdot])[e]
    &=
    e,\\
    \mathsf{realize}_\rho(
      \letkw\ x=C[\cdot]\ \inkw\ \hat e_2)[e]
    &=
    \letkw\ x=\mathsf{realize}_\rho(C)[e]
    \ \inkw\ \mathsf{realize}_\rho(\hat e_2),\\
    \mathsf{realize}_\rho(
      \ifkw\ C[\cdot]\ \thenkw\ \hat e_2\ \elsekw\ \hat e_3)[e]
    &=
    \ifkw\ \mathsf{realize}_\rho(C)[e]
    \ \thenkw\ \mathsf{realize}_\rho(\hat e_2)
    \ \elsekw\ \mathsf{realize}_\rho(\hat e_3),\\
    \mathsf{realize}_\rho(C[\cdot] + \hat e_2)[e]
    &=
    \mathsf{realize}_\rho(C)[e] + \mathsf{realize}_\rho(\hat e_2),\\
    \mathsf{realize}_\rho(\hat v + C[\cdot])[e]
    &=
    \mathsf{realize}_\rho(\hat v) + \mathsf{realize}_\rho(C)[e].
  \end{aligned}
  \]
  Other context forms are defined analogously.
\end{definition}
...
\begin{lemma}\label{lem:symbolic-step-context}
    ...
    \lambda\rho.\,
      \left(
        ...
        \lambda g.\,
          \delta_{
            \mathsf{realize}_\rho
              (C[\letkw\ x=[\cdot]\ \inkw\ \hat e_2])[g] : \tau}
      \right)
    \tag{\text{by induction hypothesis}} \\
    &=
    \sem{\sigma}_{\mathsf{env}}
...
\begin{lemma}\label{lem:symbolic-step}
...
\begin{proof}
  Let \(\hat{s}=(\sigma \mid \hat e : \tau)\). Apply
  \Cref{lem:symbolic-step-context} with the empty surrounding expression
  \(C[\cdot]=[\cdot]\):
  \begin{align}
    &\mathsf{Act}_\tau\!\left(\widehat{\sem{\hat{s} : \tau}}\right) \notag \\
    &=
    ...
    \lambda\rho.\,
      ...
      \right)
    \tag{\text{by \Cref{lem:symbolic-step-context}}} \\
    &=
    \sem{\sigma}_{\mathsf{env}}
    \monbind
    \lambda\rho.\,
...
\[
  \mathsf{mean}_{\mathsf{env}}(\sigma) \in \mathbb{R}^n.
\]
It is defined sequentially by:
\[
...
\begin{cases}
  ...
  (m,r_2,\ldots,r_n)
    & \text{if } \sigma = u \sim D(\vec a),\sigma',\\
    & \text{where } m=\mathbb{E}[D(\vec a)]
      \text{ and }
      \mathsf{mean}_{\mathsf{env}}(\sigma'[m/u])
      =(r_2,\ldots,r_n).
\end{cases}
\]
...
\begin{definition}[Lifted expected interpretation]
  The lifted expected interpretation acts on distributions over symbolic states:
  \[
    \mathsf{Exp}_\tau :
    \mathcal{D}(State_\tau) \to \mathcal{D}(Expr_\tau).
  \]
  It is defined by monadic bind against the single-state expected
  interpretation:
  \[
  \begin{aligned}
    \mathsf{Exp}_\tau(\mu)
...
\begin{equation}
  \mathsf{Exp}_\tau\!\left(\widehat{\sem{\hat{s} : \tau}}\right)
  =
  \mathsf{exp}_\tau(\hat{s})
  \monbind
  \lambda e'.\,\sem{e' : \tau}.
  \label{eq:expected-one-step-compatibility}
\end{equation}

\begin{theorem}[Expected interpretation of symbolic \(n\)-step semantics]
  Assume \Cref{eq:expected-one-step-compatibility} holds for every
  \(\hat{s} \in State_\tau\).  If
  \(\hat{s} = (\sigma \mid \hat e : \tau) \in State_\tau\), then for every
  \(n \in \mathbb{N}\),
  \[
    \mathsf{Exp}_\tau\!\left(
      \widehat{\bigsemstep{\hat{s} : \tau}{n}}
    \right)
    =
    \mathsf{exp}_\tau(\hat{s})
...
\begin{proof}
  The proof is the same induction on \(n\) as for the actual interpretation,
  using \Cref{eq:expected-one-step-compatibility} in place of
  \Cref{lem:symbolic-step}.
  ...
  \begin{align}
    ...
    \right)
    &=
    \mathsf{Exp}_\tau(\delta_{\hat{s} : \tau})
    \tag{\text{by definition of n-step symbolic semantics}} \\
    &=
    \delta_{\hat{s} : \tau}
    \monbind
    \mathsf{exp}_\tau
    \tag{\text{by definition of Exp}} \\
    &=
    \mathsf{exp}_\tau(\hat{s})
    \tag{\text{by monad left identity}} \\
    &=
  ...
  For the inductive step, assume the statement holds for \(n\) for every
  well-typed symbolic state, and let
  \(\hat{s} = (\sigma \mid \hat e : \tau) \in State_\tau\). Then
  \begin{align}
    &\mathsf{Exp}_\tau\!\left(
      \widehat{\bigsemstep{\hat{s} : \tau}{n+1}}
    \right) \notag \\
    ...
    \monbind
    \lambda \hat{s}'.\,
      \left(\mathsf{exp}_\tau(\hat{s}')
      \monbind \lambda e'.\,\sem{e' : \tau}\right)
    \tag{\text{by \Cref{eq:expected-one-step-compatibility}}} \\
    &=
    \left(
      \widehat{\bigsemstep{\hat{s} : \tau}{n}}
      \monbind
      \lambda \hat{s}'.\,\mathsf{exp}_\tau(\hat{s}')
    \right)
    \monbind
    \lambda e'.\,\sem{e' : \tau}
    \tag{\text{by monad associativity}} \\
