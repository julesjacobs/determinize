...
recorded in $\sigma$ with their means rather than sampling them.

The mean valuation determined by a symbolic sample environment is defined
recursively by
\[
\begin{aligned}
  \mathsf{means}(\emptyset)
  &=
  (),\\
  \mathsf{means}
  \bigl(\sigma,\,u\sim D(\vec a)\bigr)
  &=
  \rho[u\mapsto m],
\end{aligned}
\]
where
\[
  \rho=\mathsf{means}(\sigma),
  \qquad
  \rho\vdash\vec a\Downarrow\vec r,
  \qquad
  m=\mathbb E_{X\sim D(\vec r)}[X].
\]
...
\subsubsection{Relating original to determinized}

An $n$-step dustribution may still contain non-value expressions, so its expectation is not directly defined. Must restrict each $n$-step distribution to values.

For a float-valued distribution $\mu$ with finite expectation, let
\[
  \mathbb{E}[\mu]
  :=
...
\begin{theorem}\label{thm:interpretation-of-means}
  Let $m\in\{\E,\G\}$, and let
  \[
    s=(\sigma\mid\sym(a):\Float m)\in State_{\Float m}
  \]
  be a terminal symbolic state. Then
  \[
    \mathbb{E}[\mathsf{act}_{\Float m}(s)]
    =
    \mathbb{E}[\mathsf{exp}_{\Float m}(s)].
  \]
\end{theorem}

\begin{theorem}[Soundness n-step]
  Let $e : \tau$ be a well-typed expression. Then, for every $n$, when restricted to values
  \[
    \mathbb{E}[\bigsemstep{e : \Float{m}}{n}]
    =
    \mathbb{E}[\bigsemstep{\exptrans{e : \Float{m}} : \Float{m}}{n}]
  \]
...
\begin{proof}
  \begin{align}
    ...
    \mathbb E\left[
      \delta_{e:\Float m}
      \monbind
      \lambda e'.\,\bigsem{e':\Float m}
    \right]
    \tag{\text{by the left-unit law}}
    \\
    &=
    \mathbb E\left[
      \mathsf{act}_{\Float m}
        (\emptyset\mid\mathsf{lift}(e):\Float m)
      \monbind
      \lambda e'.\,\bigsem{e':\Float m}
    \right]
    \tag{\text{by definition of initial actual interpretation}}
    \\
    &=
    \mathbb E\left[
      \symbigsem{
        (\emptyset\mid\mathsf{lift}(e):\Float m):\Float m
      }
      \monbind
      \mathsf{act}_{\Float m}
    \right]
    \tag{\text{by \Cref{thm:actual-big-step}}}
    \\
    &=
    \int
      \mathbb E[\mathsf{act}_{\Float m}(s)]
    ...
    \tag{\text{expectation of a mixture}}
    \\
    &=
    \int
      \mathbb E[\mathsf{exp}_{\Float m}(s)]
    ...
    \tag{\text{by \Cref{thm:interpretation-of-means}}}
    \\
    &=
    \mathbb E\left[
      \symbigsem{
    ...
    \mathbb E\left[
      ...
      \lambda e'.\,\bigsem{e':\Float m}
    \right]
    \tag{\text{by \Cref{thm:expected-symbolic-big-step}}}
    \\
    &=
    \mathbb E\left[
      \delta_{\exptrans{e:\Float m}:\Float m}
      \monbind
      \lambda e'.\,\bigsem{e':\Float m}
    \right]
    \tag{\text{by definition of expected interpretation}}
    \\
    &=
    \mathbb E\left[
      \bigsem{\exptrans{e:\Float m}:\Float m}
    \right].
    \tag{\text{by the left-unit law}}
  \end{align}
\end{proof}
...
\begin{proof}
  ...
  \begin{align}
    &\mathbb E\!\left[
      \left.
        \bigsemstep{e:\Float m}{n}
      \right|_{Val_{\Float m}}
    \right]
    \notag
    \\
    &=
    \mathbb E\!\left[
      \left.
        \left(
          \symbigsemstep{
            \mathsf{init}_{\Float m}(e:\Float m)
          }{n}
          \monbind
          \mathsf{act}_{\Float m}
        \right)
      \right|_{Val_{\Float m}}
    \right]
    \tag{\text{by actual interpretation of initialized symbolic semantics}}
    \\
    &=
    \mathbb E_{
      s\sim
    ...
    \left[
      \mathbb E\!\left[
        \left.
          \mathsf{exp}_{\Float m}(s)
        \right|_{Val_{\Float m}}
      \right]
    \right]
    \tag{\text{by equality of the interpretations on values}}
    \\
    &=
    \mathbb E\!\left[
      \left.
    ...
    \mathbb E\!\left[
      \left.
        \bigsemstep{
          \exptrans{e:\Float m}:\Float m
        }{n}
      \right|_{Val_{\Float m}}
    \right].
    \tag{\text{by expected interpretation of initialized symbolic semantics}}
  \end{align}
\end{proof}
