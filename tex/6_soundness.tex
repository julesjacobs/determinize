...
$(\sigma \mid \hat e : \tau) \in State_\tau$, but replaces the samples
recorded in $\sigma$ with their means (replace in residual expression) rather than sampling them.

First, we define the following, replacing each symbolic variable in the sampling enviornment by the mean of its distribution. Distribution parameters are evaluated sequentially under the valuation induced by the preceding bindings:
...
&=
\delta_{()},\\
\sem{\sigma,u\sim D(\vec a)}_{\mathsf{means}}
&=
\sem{\sigma}_{\mathsf{means}}
\monbind
\lambda\rho.\,
\delta_{\rho\left[
u\mapsto
...
with \(\rho\vdash\vec a\Downarrow\vec r\). Note here that \(\sem{\sigma}_{\mathsf{means}}\) is a Dirac measure concentrated on a single valuation, where 
\(\sem{\sigma}_{\mathsf{means}} \in \mathcal{D}(\mathbb{R}^n)\).
