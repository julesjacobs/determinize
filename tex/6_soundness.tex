...
\begin{definition}[Expected realization]
  ...
  \begin{aligned}
    ...
    &=
    r
    &&\text{if }\rho\vdash a\Downarrow r,
    \\
    ...
    \\
    \mathsf{erealize}_\rho
      \bigl(\uniform[\E](\hat e_1,\ldots,\hat e_k)\bigr)
    &=
    \mathsf{mean}_\uniform
      \bigl(
        \mathsf{erealize}_\rho(\hat e_1),
        \ldots,
        \mathsf{erealize}_\rho(\hat e_k)
      \bigr),
    \\
    \mathsf{erealize}_\rho
      \bigl(\uniform[\G](\hat e_1,\ldots,\hat e_k)\bigr)
    &=
    \uniform\bigl(
      \mathsf{erealize}_\rho(\hat e_1),
      \ldots,
...
\begin{lemma}
  \[
    \mathsf{erealize}_{()}(\mathsf{lift}(e)) = \exptrans{e}
  \]
\end{lemma}
...
\begin{lemma}[Initial expected interpretation]
  ...
  \[
    \mathsf{exp}_\tau
    (\emptyset\mid\mathsf{lift}(e):\tau)
    =
    \delta_{\exptrans{e:\tau}}.
  \]
\end{lemma}


\begin{theorem}
  Let \(s_0=(\emptyset\mid\mathsf{lift}(e)),
\qquad
\nu_n=\symbigsemstep{s_0}{n}.\)

\(\bigsemstep{\exptrans e}n=\nu_n\monbind\mathsf{exp}.\)
