...
Now, define the big step semantics as follows:
...
This is a distribution on terminal symbolic states.
...
\begin{theorem}[Actual interpretation of symbolic big-step semantics]
  \label{thm:actual-big-step}
  If $s = (\sigma \mid \hat e : \tau) \in State_\tau$, then
  \[
    \symbigsem{s : \tau}
    \monbind
    \mathsf{act}_\tau
...
\[
\begin{aligned}
  \mathsf{mean}_{\mathsf{env}}(\emptyset)
  &=
  (),\\
  \mathsf{mean}_{\mathsf{env}}
  \bigl(\sigma,\,u\sim D(\vec a)\bigr)
  &=
  \left(
    m_1,\ldots,m_n,\,
    \mathsf{mean}_D\!\left(
      \vec a[m_1/u_1,\ldots,m_n/u_n]
    \right)
  \right),
\end{aligned}
\]
...
As in the actual interpretation, the tuple $(m_1,\ldots,m_n)$ is
identified with the valuation
\[
  [u_1\mapsto m_1,\ldots,u_n\mapsto m_n].
\]
...
\begin{definition}[Single-state expected interpretation]
  ...
  \begin{aligned}
    ...
    &=
    \delta_{\exptrans{\mathsf{realize}_{\mathsf{mean}_{\mathsf{env}}(\sigma)}
      (\hat e) : \tau}}.
  \end{aligned}
  \]
\end{definition}
...
\begin{example}
  \begin{align*}
    ...
    \bigr) \\
    &=
    \delta_{\mathsf{meanUniform}
      (0,\mathsf{meanUniform}(1,2))} \\
    &=
    \delta_{0.75}.
  \end{align*}
\end{example}
...
\begin{theorem}[Expected interpretation of symbolic big step semantics]
  \label{thm:expected-symbolic-big-step}
  If $s=(\sigma\mid\hat e:\tau)\in State_\tau$, then
  \[
    \symbigsem{s:\tau}
    \monbind
...
\begin{proof}
  TODO. Apply \Cref{thm:expected-symbolic-n-step} and take the limit as
  $n\to\infty$. The result follows from the definitions of the ordinary and
  symbolic big-step semantics and monotone convergence.
\end{proof}
