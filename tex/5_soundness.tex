...
\begin{definition}[Expectation transformation]
  ...
  *** This will be dependent on the types.
  ...
  \begin{figure}[t]
    ...
    \centering
    \begin{tabular}{l|l|l}
      \textbf{$e:\tau$} & \textbf{Condition} & \textbf{$\mathcal{E}(e:\tau)$} \\
      \hline
      $c : \floattype{m}$ &
      for float literal $c$ &
      $c$ \\
      $\uniform(a : \floattype{E}, b : \floattype{E})$ &
      -- &
      $\tfrac{a+b}{2}$ \\
      $\uniform(a : \floattype{G}, b : \floattype{G})$ &
      -- &
      $\uniform(a, b)$ \\
      $\uniform(e_1 : \floattype{m}, e_2 : \floattype{m})$ &
      -- &
      $\uniform(\mathcal{E}\langle e_1 : \floattype{m} \rangle, \mathcal{E}\langle e_2 : \floattype{m} \rangle)$ \\
      $\gaussian(m : \floattype{E}, \sigma^2 : \floattype{G})$ &
      -- &
      $m$ \\
      $\ldots$ &
      -- &
      $\ldots$ \\
      $\letkw\ x = e_1 \inkw\ e_2$ &
      -- &
      $\letkw\ x = \mathcal{E}\langle e_1 \rangle \inkw\ \mathcal{E}\langle e_2 \rangle$
      \\
    \end{tabular}
    \caption{Expectation transformation clauses.}
    \label{fig:expectation-transformation}
  \end{figure}
\end{definition}
