...
The six statements of `Spec/Inference.lean`, assembled from the lemmas of
...
* `solveInput_complete` (completeness and optimality): a typed completion solves the subtyping
  ...
  solution (Lemma P); read-back is monotone. If the completion has a float type, the decorated
  type of the draft is a float or a shape variable.
-/
...
/-- A typed completion determines a solution of inference above it. If the completion has a
float type, so does every instance of the draft's decorated type that sends the remaining shape
variables to floats. -/
theorem solveInput_complete {input : Input} {completion : Core} {T : Ty}
    ...
    ∃ s, solveInput input = .ok s ∧ AffinityLE completion (s.draft.program s.affinities) ∧
      ((∃ m, T = .float m) →
        ∃ m, (s.draft.ty.decorate s.shapes).instantiate (fun _ => .float .E) s.affinities =
          .float m) := by
  ...
  obtain ⟨S₂, sT, program⟩ := F σ₂ fun _ _ => ⟨rfl, rfl⟩
  ...
  refine ⟨_, solveInput_of hg hu ha, ?_, ?_⟩
  · rw [← program]
    exact program_le (solveAffinities_greatest ha satisfies) d
  · -- The decorated type of the draft has a shape that maps to `float` under `δ`.
    rintro ⟨m, rfl⟩
    have shape : (d.ty.decorate θ).shape.subst δ = .float := by
      have := shapeOf_sub sT
      rw [Ground.inst, shape_instantiate] at this
      rw [shape_decorate, Shape.subst_subst, funext factor]
      exact this
    show ∃ m, (d.ty.decorate θ).instantiate _ ρmax = _
    cases hty : (d.ty.decorate θ).shape with
    | var β => rw [eq_var_of_shape hty]; exact ⟨.E, rfl⟩
    | float =>
      obtain ⟨a, ha⟩ := eq_float_of_shape hty
      rw [ha]
      exact ⟨_, rfl⟩
    | _ => rw [hty] at shape; simp [Shape.subst] at shape

/-! ## The statements -/

theorem inferSound : inferSoundThm := by
  intro input program ty h
  obtain ⟨s, hs, rfl, rfl⟩ := infer_ok.1 h
  exact solveInput_typed hs _

theorem inferOptimal : inferOptimalThm := by
  intro input program ty completion h ⟨hm, T, ht⟩
  obtain ⟨s, hs, rfl, -⟩ := infer_ok.1 h
  obtain ⟨s', hs', le, -⟩ := solveInput_complete hm ht
  rw [hs, Except.ok.injEq] at hs'
  subst hs'
  exact le

theorem inferComplete : inferCompleteThm := by
  intro input completion ⟨hm, T, ht⟩
  obtain ⟨s, hs, -, -⟩ := solveInput_complete hm ht
  exact ⟨_, _, infer_ok.2 ⟨s, hs, rfl, rfl⟩⟩

theorem inferFloatSound : inferFloatSoundThm := by
  intro input program affinity h
  obtain ⟨hm, ht⟩ := inferSound input program _ h
  exact ⟨hm, ht.sub (sub_float_E affinity)⟩

theorem inferFloatComplete : inferFloatCompleteThm := by
  intro input completion hm ht
  obtain ⟨program, ty, h⟩ := inferComplete input completion ⟨hm, _, ht⟩
  exact ⟨program, ty, h, inferOptimal input program ty completion h ⟨hm, _, ht⟩⟩

theorem inferFloatTyped : inferFloatTypedThm := by
  intro input completion hm ht
  obtain ⟨s, hs, -, float⟩ := solveInput_complete hm ht
  obtain ⟨m, hty⟩ := float ⟨_, rfl⟩
  have typed := (solveInput_typed hs fun _ => .float .E).2
  rw [hty] at typed
  exact ⟨_, _, infer_ok.2 ⟨s, hs, rfl, rfl⟩, typed.sub (sub_float_E m)⟩
