...
private def candidateFor (text : String) (subject : Subject := .source) : IO Candidate := do
...
private def accepted (candidate : Candidate) : Bool :=
  (checkModel candidate.source candidate.subject candidate).isSome
...
def modelReplay : IO Unit := do
  for text in [
    ...
    assert (accepted (← candidateFor text)) s!"machine replay: {text}"
  let three ← candidateFor "3"
  ...
  for i in [:coin.rows.size] do
    ...
    | .returned reward =>
        ...
        assert (!accepted (setRow coin i {row with kind := .rejected, evidence := .rejected})) "changed outcome"
    | _ => pure ()
  ...
  | .complete candidate => assert (!accepted candidate) "free variable in unused closure"
  | _ => throw (IO.userError "scope fixture must finish exploration")
  let broken : Candidate := ⟨.bvar 0,.source,0,#[.eval (.bvar 0) [] []],
