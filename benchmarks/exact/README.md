...
* fully: means ALL randomness is eliminated, the resulting program is deterministic
* partially: at least one random construct is eliminated, but some randomness remains.
* no: no random construct can be safely eliminated.

In **fully discrete after deteterminization?**, `✓ (deterministic)` means determinization removes all randomness, while `✓` means only discrete random choices remain. The cell is blank when determinization is unavailable or leaves a continuous distribution.

| Program | Determinize? | Storm? | Reason | continuous? | fully discrete after determinization? | Citation |
| --- | --- | --- | --- | --- | --- | --- |
...
| `bayesian.det` | Yes, partially | No | Draws assigned to `s` and `l` do not affect control and can become means; `i`, `d`, and `g` remain random because they select branches. The infinite loop makes `n` unbounded. |  | ✓ | [k-induction] |
...
| `fcall.det` | Yes, fully | No | The update can become `x + 0.5`, but the loop never terminates and `x` is unbounded. |  | ✓ (deterministic) | [k-induction] |
...
| `bin01_0.det` | Yes, fully | Yes | Each choice can become the expected update `x + 0.5 * y`; the fixed `n` iterations give a finite deterministic target. |  | ✓ (deterministic) | [EXIST] |
| `detm1_0.det` | Yes, trivially | Yes | The program is already deterministic, and `x` increases only until it exceeds `10`. |  | ✓ (deterministic) | [EXIST] |
...
| `sum01_0.det` | Yes, fully | Yes | Each choice can become the expected update `x + 0.5 * n`; `n` then decreases deterministically to zero. |  | ✓ (deterministic) | [EXIST] |
| `add-uniform-with-counter-large-bounded.det` | No | No | The same `cur` controls the threshold branch and contributes to the result, forcing it to G. Without unsupported conditional splitting, its continuous values prevent a finite Storm model. | ✓ |  | [GuBPI] |
| `add-uniform-with-counter-large-unbounded.det` | No | No | The same `cur` controls the threshold branch and contributes to the result, forcing it to G. The retained continuous draw and unbounded recursion prevent finite-state translation. | ✓ |  | [GuBPI] |
...
| `cav-example-5-simplified.det` | No | No | `bet` syntactically flows through recursive `money`, which is tested by the guard, so inference forces it to G. The pass does not prove the cancellation, and the continuous intermediate state is not finite-state. | ✓ |  | [GuBPI] |
...
| `nealsFunnel.det` | Yes, partially | No | The `y` draws can become their mean `0`, but sampled `x` remains G because it determines their Gaussian variances. The retained continuous `x` prevents finite-state exploration. | ✓ |  | [GuBPI] |
...
| `pdmb-v3.program` | Yes, trivially | No | No sampling occurs: `r_0`–`r_4` are free real inputs. Some inputs cause divergence or unbounded state, so this is not a closed finite model. |  | ✓ (deterministic) | [Wang et al.] |
| `pdmb-v4.program` | Yes, trivially | No | No sampling occurs: `r_0`–`r_5` are free real inputs. The loop or `p_dis` may be unbounded, so this is not a closed finite model. |  | ✓ (deterministic) | [Wang et al.] |
...
| `example_1.prog` | No | No | Continuous `r` controls the loop guard, and the fair probabilistic command controls both the tick and `y`. Unsupported branch averaging is required to eliminate either choice. | ✓ |  | [Chatterjee et al.] |
| `example_3.prog` | No | No | Continuous `r` controls the loop guard, while the fair command branch remains inside nondeterministic control. The current pass cannot average that branch. | ✓ |  | [Chatterjee et al.] |
...
| `pollutant_disposal.prog` | No | No | The `0.6` command choice remains random, and each continuous sample affects cost or guard variable `n`. The pass does not exploit symmetry between the branch bodies. | ✓ |  | [Chatterjee et al.] |
| `convoy.det` | No | No | `Uniform(-2,2)` changes `a1`, then `v1`, `x1`, and guard `x1-x2`; it cannot become its mean. `while true` leaves unbounded continuous state and no return. | ✓ |  | [Chakarov et al. (CAV 2013)] |
| `dreckon.det` | Yes, partially | Yes | Step sizes become `1.5` and sensor noises become `0`, but categorical `cmd` remains because it controls the switch. The fixed `N = 500` horizon still yields a finite discrete model. | ✓ | ✓ | [Chakarov et al. (CAV 2013)] |
| `invpend.det` | Yes, fully | Yes | The bounded loop is affine: initial means are `-4`, `2.5`, `3`, and `0`, and both disturbance means are `0`. This gives one finite deterministic trajectory. | ✓ | ✓ (deterministic) | [Chakarov et al. (CAV 2013)] |
| `pack.det` | Yes, partially | Yes, with reward encoding | Weight noises become `0.1` or `0.05`; object type stays random because it controls bounded counters and exit. Attempts, count, and weight become rewards. | ✓ | ✓ | [Chakarov et al. (CAV 2013)] |
| `roulette.det` | No | No | `rand(5,10)` and every flip update guard variable `money`; `i` counts an unbounded number of rounds. Reachable money is continuous and unbounded. | ✓ |  | [Chakarov et al. (CAV 2013)] |
...
[Chakarov et al. (CAV 2013)]: https://plv.colorado.edu/papers/martingales-cav13.pdf
